...
export default function DialogEditKhentapthe({
  open,
  onCloseDialogEdit,
  item,
  onSubmit,
  ...
  const [capkhenBefore, setCapkhenBefore] = useState('');

  useEffect(() => {
    if (item) {
      setCapkhenBefore(item.cap);
      setValue("nam", item.nam, { shouldValidate: true });
      setValue("noidung", item.noidung, { shouldValidate: true });
      setValue("taptheduockhenthuong", { value: item.tapthe_id, label: item.tapthe }, { shouldValidate: true });
    }
  }, [item]);
  ...
  const handleFormSubmit = async (values) => {
    if (onSubmit) {
      const data = { ...values, id_edit: item._id, capkhenBefore };
      await onSubmit(data);
      onCloseDialogEdit();
    }
  };
      ...
      >
        ...
        >
          ...
          <span className="text-white text-[18px]">
            Chỉnh sửa danh hiệu thi đua tập thể
          </span>
            ...
            <form className='mt-2 mx-8' onSubmit={handleSubmit(handleFormSubmit)}>
              <div className='flex flex-wrap xl:flex-row flex-col flex-1 p-2'>
                <div className='flex-col md:basis-1/3 flex flex-1 px-1'>
                  <label className='underline font-semibold'><span className='text-red-600 font-semibold'> (*)</span> Năm đạt danh hiệu: </label>
                  <input {...register("nam", { required: true })} disabled type="text" className='outline-none my-4 border rounded-md p-2 border-neutral-600
                ...
                <div className='flex-col md:basis-full flex flex-1 px-1'>
                  <label className='underline font-semibold'><span className='text-red-600 font-semibold'> (*)</span>Tập thể đạt danh hiệu: </label>
                  <Controller
                    control={control}
