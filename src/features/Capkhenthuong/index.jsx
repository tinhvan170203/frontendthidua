...
const CapkhenthuongComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        handleLoading(true);
        let res = await capkhenthuongApi.getCapkhenthuongs();
        setKhois(res.data);
        handleLoading(false);
      } catch (error) {
        if (
  ...
  const onSubmit = async (values) => {
    ...
    handleLoading(true);
    try {
      let res = await capkhenthuongApi.addCapkhenthuong(data);
  ...
  const handleSubmitEdit = async (values) => {
    ...
    handleLoading(true);
    try {
      let res = await capkhenthuongApi.editCapkhenthuong(data);
      setKhois(res.data.khois)
      handleLoading(false);
      toast.success(res.data.message, {
        position: "top-right",
  ...
  const handleConfirmDelete = async () => {
    handleLoading(true);
    try {
      let res = await capkhenthuongApi.deleteCapkhenthuong(openDialogDelete.id_Delete);
  ...
  return (
    <div className='pr-2' data-aos="fade-left" data-aos-once="true">
      <div className='my-2'>
        <h4 className='uppercase font-bold text-blue-800 text-sm md:text-[18px]'>Quản lý các cấp khen thưởng</h4>
      </div>

      <form className='mt-2 px-6 py-4 shadow-gray-400 shadow-md' onSubmit={handleSubmit(onSubmit)}>
        <div className='flex-col flex'>
          <label className='underline font-semibold'><span className='text-red-600 font-semibold'> (*)</span> Cấp khen thưởng: </label>
          <input {...register("capkhen", { required: true })} type="text" className='outline-none my-4 border rounded-md p-2 border-neutral-600
...
export default CapkhenthuongComponent
