...
import TableChitietThiduanam from '../components/TableChitietThiduanam';
...
const ThiduanamThongke = () => {
  ...
  const handleFetchThiduanam = async () => {
    //  console.log(watch('donvi'))
    try {
      handleLoading(true);
      let res = await thongkeApi.getThiduanams({ vaitro, nam: year, donvi: watch('donvi').value });
      setYearDisplay(year);
  ...
  return (
    <div className='pr-2' data-aos="fade-left" data-aos-once="true">
      <div className='my-2'>
        <h4 className='uppercase font-bold text-blue-800 text-sm md:text-[18px]'>Thống kê, theo dõi kết quả thi đua năm của cán bộ, chiến sĩ công tác</h4>
      </div>
 ...
 <p className='text-red-600 font-medium text-center mt-8'>{`Biểu đồ kết quả xếp loại thi đua năm cán bộ, chiến sĩ`}</p>
      <div className='flex mb-8 mt-2 justify-center items-center shadow-lg shadow-slate-300 py-8'>
        <div className='md:basis-1/4'>
          <DoughnutChart text={`Biểu đồ kết quả thi đua năm của cán bộ, chiến sĩ công tác năm ${yearDisplay} ${chidoanDisplay.label}`}
            dataHTNV={dataHTNV} dataHTTNV={dataHTTNV} dataHTXSNV={dataHTXSNV} dataKHTNV={dataKHTNV} dataNotValue={dataNotValue} total={dataBang.length} />
        </div>
      </div>
 ...
 <div className='shadow-lg  mb-8 pb-8 shadow-slate-300 '>
      <div className='flex mt-4'>
       ...
       <p className='text-blue-600 font-medium text-center mt-2'>Thống kê số lượt xếp loại thi đua năm</p>
      </div>

      <div className='mt-4'>
        <p className='text-center text-lg my-8 font-semibold'>Bảng xếp loại thi đua năm của cán bộ, chiến sĩ đang công tác năm {yearDisplay}</p>
        ...
        <div className='mt-4 text-end mb-2 md:space-x-2 space-y-2 md:space-y-0 md:block flex flex-col items-center'>
          <Button variant='contained' color='success' size='small'>
            <CSVLink data={excelExport} headers={headers} filename={`xeploaithiduanam`}>
              <FileDownloadIcon />
        ...
        <div className='mb-8'>
          <TableChitietThiduanam list={dataDisplay} />
        </div>
      </div>
