...
const TonghopThiduanam = () => {
  ...
  const handleFetchData = async () => {
    ...
    handleLoading(true)
    try {
      let res = await thongkeApi.getBangThiduanams({ tunam, dennam, donvis: accounts_send });
  ...
  return (
    <div className='pr-2' data-aos="fade-left" data-aos-once="true">
      <div className='my-2'>
        <h4 className='uppercase font-bold text-blue-800 text-sm md:text-[18px]'>Thống kê, theo dõi kết quả thi đua năm của cán bộ, chiến sĩ trong khoảng thời gian năm</h4>
      </div>
      ...
      <p className='text-red-600 font-medium text-center mt-8'>{`Biểu đồ kết quả xếp loại thi đua năm từ năm ${tunam.value} đến năm ${dennam.value}`}</p>
      <div className='flex mb-8 mt-2 justify-center items-center shadow-lg shadow-slate-300 py-8'>
        <div className='md:basis-1/4'>
          <DoughnutChart text={`Biểu đồ kết quả thi đua năm của cán bộ, chiến sĩ từ năm ${tunam.value} đến năm ${dennam.value}`}
     ...
     <div className='shadow-lg  mb-8 pb-8 shadow-slate-300 '>
      ...
      <div className='mt-4'>
        <p className='text-center text-lg mt-8 font-semibold'>Danh sách xếp loại thi đua năm của cán bộ, chiến sĩ</p>
        <h3 className='text-center italic'>(từ năm {tunam.value} đến năm {dennam.value})</h3>

        <h3 className='text-end my-4'>Tổng số: <span className='text-xl font-bold text-red-600'>{dataDisplay.length}</span> cán bộ, chiến sĩ</h3>
...
export default TonghopThiduanam