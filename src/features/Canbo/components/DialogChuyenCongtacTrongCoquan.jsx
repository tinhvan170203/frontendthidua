...
export default function DialogChuyenCongtacTrongCoquan({
  open,
  item,
  donviList,
  ...
  useEffect(() => {
    setDonvis(donviList.slice(1));
  }, [donviList]);


  const handleFormSubmit = async (values) => {
    if (onSubmit) {
      const data = { ...values,ngayvedonvi,ghichu, donvi, id: item._id };
      await onSubmit(data);
      onCloseDialogChuyenCongtac();
    }
  };
      ...
      >
        ...
        >
          ...
          <span className="text-white text-[16px]">
            Chuyển đội nghiệp vụ, tổ công tác trong nội bộ đơn vị đối với đồng chí "{item?.hoten}"
          </span>
