...
import TableKhencanhan from '../components/TableKhencanhan';
import DialogAddKhencanhan from '../components/DialogAddKhencanhan';
import DialogEditKhencanhan from '../components/DialogEditKhencanhan';
...
const Khencanhan = () => {
  ...
  const handleFormSearchSubmit = async (values) => {
    ...
    try {
      handleLoading(true);
      let res = await khenthuongApi.searchKhencanhans(canbo._id, data);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        handleLoading(true);
        let res = await khenthuongApi.getKhencanhans(canbo._id);
  ...
  const handleSubmitAdd = async (values) => {
    ...
    handleLoading(true);
    try {
      let res1 = await khenthuongApi.addKhencanhan(data);
      let res = await khenthuongApi.getKhencanhans(canbo._id);
      setKhenthuongs(res.data)
      setExcelExport(res.data.map((i, index) => ({
        stt: index + 1,
  ...
  const handleConfirmDelete = async () => {
    handleLoading(true);
    try {
      let res = await khenthuongApi.deleteKhencanhan(openDialogDelete.id_Delete);
  ...
  const handleSubmitEdit = async (values) => {
    handleLoading(true);
    try {
      let res = await khenthuongApi.editKhencanhan(values);

      let res1 = await khenthuongApi.getKhencanhans(canbo._id);
  ...
  return (
    <div className='pr-2' data-aos="fade-left" data-aos-once="true">
      <div className='my-2'>
        <h4 className='uppercase font-bold text-blue-800 text-sm md:text-[18px]'>Quản lý khen thưởng đối với cá nhân trong đơn vị</h4>
      </div>
      ...
      <div className='shadow-md shadow-gray-500 rounded-md pb-4'>
          ...
          <>
            <div className='mt-8 text-end md:space-x-2 space-y-2 md:space-y-0 md:block flex flex-col items-center'>
              ...
              <Button variant='contained' color='success'>
                <CSVLink data={excelExport} headers={headers} filename={`danhsachkhenthuong_${canbo?.hoten}_${watch('tungay')}_${watch('denngay')}`}>
            ...
            {display && (
              <form onSubmit={handleSubmit(handleFormSearchSubmit)} className='my-2 mt-4 shadow-gray-400 shadow-md p-4 rounded-lg' data-aos="zoom-in-down" data-aos-once="true">
                <div className='flex justify-between'>
                  <h5 className='text-[18px] font-semibold'>Tìm kiếm khen thưởng cá nhân</h5>
        ...
        {canbo && (
          <div className='mt-6'>
            <h3 className='text-center text-blue-600 my-8 text-xl'>Danh sách khen thưởng cá nhân của đồng chí {canbo?.bachamPopulate[0].bacham} {canbo?.hoten}</h3>

              <div className='mb-4 pl-4'>
                <span className='font-semibold'>Tổng số: <span className='text-2xl text-red-600 font-semibold'>{khenthuongs.length}</span> lượt khen thưởng đối với cá nhân</span>
              </div>
